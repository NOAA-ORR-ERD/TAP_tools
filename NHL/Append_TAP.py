...
This script adds a new location to an existing TAP setup.
The data required to set up and build TAP cubes + site.txt file should be in here and identical to the original Setup_TAP
...
AppendNo = '1'  # When running several parallel append scripts, this should be unique
...
RunPyGnome = True
BuildCubes = True
BuildMudFlatCubes = True
BuildSite_Viewer = True
...
nstarts = [15]  # range(0,len(Locations)) #[15,9] #
StartSites = []
for i in nstarts:
...
TrajectoriesPath = 'Trajectories_n' + str(NumLEs) + '_' + AppendNo  # relative to RootDir
MapName = Project + ' TAP'
CubesPath = 'Cubes_n' + str(NumLEs) + '_' + AppendNo
...
file = open(os.path.join(RootDir, "BinLocs_"+ AppendNo +".txt"), 'w')
file.close()
...
for i, StartSite in enumerate(StartSites):
    ...
    if BuildSite_Viewer:
        import Add_Site_Viewer
        Add_Site_Viewer.main(RootDir, AppendNo, StartSite, TAPViewerPath, CubesPath, Seasons, CubesRootNames[0], BuildMudFlatCubes)
